...
import {
  isQuestionBookmarked, toggleBookmarkQuestion,
...
function PracticeTab({ questions }: { questions: any[] }) {
  ...
  useEffect(() => {
    ...
    questions.forEach(q => { map[q.id] = isQuestionBookmarked(q.id) })
    setBookmarked(map)
  }, [questions])
  ...
  function handleBookmark() {
    const now = toggleBookmarkQuestion(q.id)
...
function MyNotesTab({ topicId }: { topicId: string }) {
  ...
  useEffect(() => { setNote(getStudentNote(topicId)) }, [topicId])

  function handleSave() {
    saveStudentNote(topicId, note)
    setSaved(true)
  ...
  return (
    <div>
      <p style={{ color: 'var(--text2)', fontSize: '14px', marginBottom: '16px' }}>
        Jot down your personal notes for this topic. Saved locally in your browser.
      </p>
      <textarea
        value={note}
