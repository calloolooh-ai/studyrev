...
export default function TopicTabs({ topic, notes, questions }: Props) {
  ...
  const tabs = [
    ...
    { id: 'practice' as const, label: '✏️ Practice' },
    { id: 'quiz' as const, label: '⏱ Quiz' },
    { id: 'mynotes' as const, label: '📝 My Notes' },
  ]
  return (
    <div>
      ...
      <div className="fade-in" key={tab}>
        ...
        {tab === 'quiz' && <QuizTab questions={questions} topicId={topic.id} />}
    ...
    <div>
      ...
      {revealed ? (
        ...
        <button
          ...
          style={{ width: '100%', justifyContent: 'center', marginBottom: '20px', padding: '14px' }}
...
function QuizTab({ questions, topicId }: { questions: any[]; topicId: string }) {
  ...
  function startQuiz() {
    const shuffled = questions.slice().sort(() => Math.random() - 0.5).slice(0, QUIZ_COUNT)
    setPool(shuffled)
    setCurrent(0)
    setRevealed(false)
  ...
  function answer(result: 'correct' | 'wrong') {
    ...
    setAnswers(updated)
    setTimeout(() => {
      if (current + 1 >= QUIZ_COUNT) {
        setState('done')
        const score = updated.filter(x => x === 'correct').length
  ...
  if (state === 'done') {
    ...
    const label = pct >= 80 ? 'Excellent!' : pct >= 60 ? 'Good job' : pct >= 40 ? 'Keep going' : 'Needs work'
    ...
    return (
      <div style={{ textAlign: 'center', padding: '40px 20px' }}>
        ...
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: '4rem', fontWeight: 700, color, marginBottom: '8px' }}>
          {score}/{pool.length}
        </div>
        <div style={{ fontSize: '1.2rem', color, fontFamily: 'var(--font-display)', marginBottom: '6px' }}>{label}</div>
    ...
    <div>
      ...
      <div className="card" style={{ marginBottom: '16px', minHeight: '160px' }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'var(--text3)', marginBottom: '12px', textTransform: 'uppercase' }}>
          Question {current + 1}
          {q.marks && (
            <span className="badge badge-amber" style={{ marginLeft: '10px' }}>{q.marks}m</span>
          )}
        </div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            <button
              ...
              className="btn"
              style={{ flex: 1, justifyContent: 'center', padding: '14px', background: 'var(--red-dim)', color: 'var(--red)', border: '1px solid rgba(255,77,106,0.3)' }}
            >
              Incorrect
            </button>
            <button
              ...
              className="btn"
              style={{ flex: 1, justifyContent: 'center', padding: '14px', background: 'var(--green-dim)', color: 'var(--green)', border: '1px solid rgba(0,255,157,0.3)' }}
            >
              Correct
            </button>
          </div>
