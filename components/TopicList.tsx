...
import { getCompletedTopics, toggleTopicComplete, isTopicBookmarked, toggleBookmarkTopic } from '@/lib/storage'
...
export default function TopicList({ topics, subjectName }: { topics: any[]; subjectName: string }) {
  ...
  useEffect(() => {
    ...
    setBookmarked(topics.filter(t => isTopicBookmarked(t.id)).map(t => t.id))
  }, [topics])
  ...
  function handleComplete(e: React.MouseEvent, topicId: string) {
    e.preventDefault()
    e.stopPropagation()
    const nowDone = toggleTopicComplete(topicId)
  ...
  function handleBookmark(e: React.MouseEvent, topicId: string) {
    e.preventDefault()
    e.stopPropagation()
    const nowBookmarked = toggleBookmarkTopic(topicId)
